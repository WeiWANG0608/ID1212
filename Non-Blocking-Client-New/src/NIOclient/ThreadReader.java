        ...
        while (true) {  
            try {  
                ...
                if (stringByte.length()>0)
                {
                    ...
                    String rcvIP = NioClient.getString(rcvCTX,"/",")");
                    String localIP =SOCKET_CHANNEL.getLocalAddress().toString()+")";
                    localIP = NioClient.getString(localIP,"/",")");
                    if(rcvIP.equals(localIP))
                    {
                        String endLabel = NioClient.getString(rcvCTX,"{","}");

                        if(endLabel.equals("end")){
                            String ctx = NioClient.getString(rcvCTX,"[","]");
