...
public class ServerThread extends Thread implements Runnable{  
    ...
    private void getConn(SelectionKey key) throws IOException {  
        ...
        userIP = NioServer.getString(userIP,"/",")");
  ...
  //      System.out.print(sc.socket().getRemoteSocketAddress() + " ");  
        ...
        {
        String rcv = stringByte.toString();
        String send = null;
                
                System.out.println("server recevied:"+ rcv);
                String continueFLG = NioServer.getString(rcv,"[","]");
                        ...
                        //and start a new round.
                        {
                            ...
                            for(int i=0;i<NioServer.USER_BASE.size();i++)
                            {
                                ...
                                if(rcvIP.equals(baseIP))
                                {
                                    ...
                                    System.out.println(NioServer.USER_BASE);
                                    send = null;
                                    send ="/"+baseIP+")"+"{on}"+"["+"New round: Please guess a letter"+
                                            userUnderline+" <"+wordLength+" letters>"+"]";
                                    System.out.println("server sent:"+ send);
                                    sc.write(ByteBuffer.wrap(send.getBytes()));
                                    break;
                                }
                        ...
                        //the score to the client and delete all the user data.
                        {
                            ...
                            for(int i=0;i<NioServer.USER_BASE.size();i++)
                            {
                                ...
                                if(rcvIP.equals(baseIP))
                                {
                                    ...
                                    NioServer.USER_BASE.remove(i);
                                    send = null;
                                    send ="/"+baseIP+")"+"{end}"+"[" +"Thanks for playing"+
