...
public class DataBase {
    ...
    public static synchronized String dataProcess(String rcv){
        //Extracting the client's ip address by method getCTX
        String rcvIP = NioServer.getString(rcv,"/",")");
        String send = null;
        //Extracting the payload which is encapsulated in "[]".
        String CTX = NioServer.getString(rcv,"[","]");
        ...
        for(int i=0;i<NioServer.USER_BASE.size();i++)
        {
            ...
            String baseIP = NioServer.getString(temp,"/",")");
            /*
            ...
            */ 
            ...
            {
                String userRecord = NioServer.getString(temp,"!","?");
                int userRecord_n = Integer.parseInt(userRecord);
                String userAttempt = NioServer.getString(temp,"<",">");
                int userAttempt_n = Integer.parseInt(userAttempt);
                String userUnderline = NioServer.getString(temp,"[","]");
                String userWord = NioServer.getString(temp,"{","}");
                String result = dataCompare(userWord,userAttempt_n,userRecord_n,CTX,userUnderline);
